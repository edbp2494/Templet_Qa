// ─────────────────────────────────────────────────────────────────────────────
// TC: TC-SHEETS-LIST-ACTIONS-RESPONSE-003
// Plataforma: Sheets | Área: objects
// Descripción: Hace click en cada acción del menú de List View y valida señal de respuesta UI.
// Suites: Master/Full-Regression, Platforms/Full-Validation-All-Platforms, Platforms/Super-Suite-Validation, Platforms/Sheets/Sheets-Full-Regression-ReuseDriver, Platforms/Sheets/Sheets-List-Actions-Response-ReuseDriver
...
String caseId = 'TC-SHEETS-LIST-ACTIONS-RESPONSE-003'
...
def evaluateActionResponse = { String actionId, Map beforeSnap, Map afterSnap ->
    ...
    switch (actionId) {
        ...
        case 'delete':
            return (modalDelta > 0 || toastDelta > 0 || rowDelta != 0 || genericSignal) ? [ok: true] : [ok: false, soft: false, reason: 'Delete sin confirmación visible']
        case 'rename':
            return (modalDelta > 0 || toastDelta > 0 || genericSignal) ? [ok: true] : [ok: false, soft: false, reason: 'Rename sin modal/confirmación']
        case 'move':
            return (modalDelta > 0 || toastDelta > 0 || genericSignal) ? [ok: true] : [ok: false, soft: false, reason: 'Move sin modal/confirmación']
        case 'duplicate':
            return (toastDelta > 0 || rowDelta != 0 || genericSignal) ? [ok: true] : [ok: false, soft: false, reason: 'Duplicate sin señal de cambio']
        case 'edit':
            return (urlChanged || modalDelta > 0 || genericSignal) ? [ok: true] : [ok: false, soft: false, reason: 'Edit sin navegación ni modal']
        default:
...
try {
    ...
    Map selectedClient = CommonKeywords.selectPreferredOption('Sheets/Filters/select_client', clientCss, 'BRAVA', 10)
    ...
    actions.each { action ->
        ...
        if (!Boolean.TRUE.equals(menuState.opened)) {
            warnings.add("No se pudo abrir menú para acción ${action.id} (${menuState.reason}), se intenta acción directa")
        }
        ...
        if (!Boolean.TRUE.equals(clickResult.clicked)) {
            failures.add("No se pudo hacer click en acción ${action.id} (ni menú ni acceso directo)")
            closeTransientUi()
            return
        }
        ...
        Map eval = evaluateActionResponse(action.id.toString(), beforeSnap, afterSnap)
        if (!Boolean.TRUE.equals(eval.ok)) {
            if (Boolean.TRUE.equals(eval.soft)) {
                warnings.add("${action.id}: ${eval.reason}")
            } else {
                failures.add("${action.id}: ${eval.reason}")
            }
        } else {
            if (!subwindowOpened) {
                warnings.add("${action.id}: respondió pero sin subventana modal visible")
            }
        }
    ...
    if (failures.isEmpty()) {
        if (!warnings.isEmpty()) {
            KeywordUtil.logInfo('[WARN] ' + warnings.join(' | '))
        }
