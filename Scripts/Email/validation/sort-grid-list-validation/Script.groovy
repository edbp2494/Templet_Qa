...
String caseId = 'TC-EMAIL-SORT-VALIDATION-001'
...
def getVisibleDocumentTitles = {
    List titles = (List) WebUI.executeJavaScript("""
        ...
        var cardSelectors = '.document-item, .email-item, .message-item, .thumbnails-boxes .thumbnail-box, .thumbnails-boxes .card, .documents-list .row, [class*="item"], [class*="card"]';
        var cards = Array.from(document.querySelectorAll(cardSelectors)).filter(function(card) {
            if (!(card instanceof HTMLElement)) return false;
            var st = window.getComputedStyle(card);
            return st.display !== 'none' && st.visibility !== 'hidden' && card.offsetParent !== null;
        }).map(function(card) {
            var titleEl = card.querySelector('h6.title-card[data-title], h6[data-title], h6.title-card, h6, [data-title]');
...
def hasNoDocuments = {
    Boolean found = (Boolean) WebUI.executeJavaScript("""
        ...
        return Array.from(document.querySelectorAll('body *')).some(function(el) {
            ...
            return messages.some(function(msg) { return text.indexOf(msg) >= 0; });
        });
    """, null)
...
def validateSort = { String viewLabel ->
    ...
    ['Newest', 'Oldest', 'A to Z', 'Z to A'].each { String sortLabel ->
        ...
        if (titles.isEmpty()) {
            ...
            KeywordUtil.logInfo("[${viewLabel}] Sort '${sortLabel}' → ${titles.size()} títulos")
...
try {
    ...
    for (Map candidate : candidates) {
        ...
        if (isSortEnabled()) {
            ...
            KeywordUtil.logInfo("[SETUP] Initiative con Sort: ${iText}")
            break
        }
    }
    
    if (selectedInitiative == null) {
        failures.add('[SETUP] Ninguna initiative con Sort habilitado')
        KeywordUtil.markFailed(failures.join(' | '))
    }
    ...
    if (failures.isEmpty()) {
        KeywordUtil.markPassed("${caseId} OK. Sort funciona en Email.")
    } else {
