// ─────────────────────────────────────────────────────────────────────────────
// TC: TC-EMAIL-TEST-PROD-OBJECTS-005
// Plataforma: Email | Área: (raíz)
// Descripción: Comparación de objetos entre TEST y PROD para Email.
// Suites: CrossPlatform/Compare-Test-Prod, Master/Full-Regression, Platforms/Email/Smoke
...
// URL de TEST con ruta /admin/manager.php para llegar al mismo panel que PROD
String testUrl       = CommonKeywords.getRequiredGlobal('EMAIL_TEST_URL', 'https://emails-test.templet.io/admin/manager.php')
String prodUrl       = CommonKeywords.getRequiredGlobal('EMAIL_PROD_URL', 'https://email.templet.io/admin/manager.php')
...
if (mismatches.isEmpty()) {
	KeywordUtil.markPassed('TC-EMAIL-TEST-PROD-OBJECTS-005 OK. Los objetos estables de Email TEST y PROD son equivalentes.')
} else {
	String mismatchSummary = mismatches.join(' | ')
	KeywordUtil.logInfo('Diferencias encontradas: ' + mismatchSummary)
	KeywordUtil.markFailed('TC-EMAIL-TEST-PROD-OBJECTS-005 encontró diferencias entre TEST y PROD: ' + mismatchSummary)
}
