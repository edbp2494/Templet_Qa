...
/**
 * TC-EMAIL-FILTERS-CLIENT-INITIATIVE-SORT-007
 * Tipo: Funcional / Regresión
 * Plataforma: Email TEST
...
String snapDir = System.getProperty('user.dir') + '/Reports/Screenshots/TC-EMAIL-FILTERS-007'
...
def chooseInitiativeWithEnabledSort = {
	...
	List<Map> initiatives = CommonKeywords.readSelectOptionsWhenReady(INITIATIVE_CSS, 20, 1)
	...
	for (Map opt : candidates) {
		...
		if (!setSelectValueByCss(INITIATIVE_CSS, value)) {
			logWarning("[INITIATIVE] No se pudo seleccionar value='${value}' en ${INITIATIVE_CSS}")
			continue
		}
...
def chooseClient = {
	...
	def clickInitiativeSelect = {
		try {
			...
			logWarning("[CLIENT] No se pudo clickear Initiative tras elegir client: ${e.message}")
			return false
		}
	}
...
if (!browserAlreadyOpen) {
	...
	// Pantalla "No volver a mostrar" — opcional según sesión
...
validateSelectItems('Sheets/Filters/select_client', CLIENT_CSS, 'CLIENT', true)
snap('01_login_ok')
...
boolean initiativeClickable = Boolean.TRUE.equals((Boolean) WebUI.executeJavaScript('''
	var sel = document.querySelector(arguments[0]);
	if (!sel) return false;
	return !sel.disabled;
''', [INITIATIVE_CSS]))
KeywordUtil.logInfo("[CHECK] Select Initiative habilitado tras elegir Client: ${initiativeClickable ? 'SI ✓' : 'NO ✗'}")
if (!initiativeClickable) {
	KeywordUtil.markFailedAndStop("[CHECK] No quedó habilitado ${INITIATIVE_CSS} tras seleccionar Client")
}
...
KeywordUtil.markPassed('TC-EMAIL-FILTERS-007: Client → Initiative → Sort validados. Capturas en ' + snapDir)
